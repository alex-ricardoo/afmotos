import { MercadoPagoConfig, Payment } from 'mercadopago-v2';
import {
...
/**
 * Adapter implementation for Mercado Pago SDK v2 (mercadopago@2.12.0)
 * Replicates the proven contract and execution pattern from Moura's Pizzas.
 */
export class MercadoPagoPaymentProviderV2 implements MercadoPagoPaymentProvider {
  ...
  constructor(accessToken?: string) {
    ...
    if (!token) {
      throw new Error('MERCADO_PAGO_ACCESS_TOKEN is required for MercadoPagoPaymentProviderV2');
    }
  ...
  async createCardPayment(input: CreateCardPaymentInput): Promise<CreateCardPaymentResult> {
    try {
      ...
      const errorObj = err as {
        status?: number;
        message?: string;
        cause?: { status?: number; message?: string; description?: string };
      };

      const statusCode = errorObj?.status ?? errorObj?.cause?.status ?? 500;
      const message =
        errorObj?.cause?.description ||
        errorObj?.cause?.message ||
        errorObj?.message ||
        'Error processing payment in Mercado Pago v2';
