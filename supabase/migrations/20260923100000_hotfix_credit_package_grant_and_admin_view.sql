...
-- 5. Recreate Unified Administrative View with LEFT JOINs and append-only package columns
-- PostgreSQL requires CREATE OR REPLACE VIEW to strictly preserve the names, types,
-- and order of all existing columns (1 to 44). New columns are appended starting at position 45.
CREATE OR REPLACE VIEW public.admin_payment_consultations_view AS
SELECT
    -- 1 a 10: Colunas de Transação
    ...
    -- 11 a 18: Dados da Consulta Veicular (consultation_id estritamente na posição 11)
    ...
    -- 19 a 22: Dados do Cliente (resolução unificada)
    ...
    -- 23 a 32: Dados do Job de Entrega
    ...
    -- 42 a 44: Indicadores Operacionais Computados (NULL-safe com LEFT JOIN e purpose)
    CASE
      WHEN pt.purpose = 'credit_package' OR pt.consultation_id IS NULL THEN FALSE
    ...
    CASE
      ...
      WHEN pt.status = 'approved' 
           AND (cpc.status IS DISTINCT FROM 'completed' OR cpc.vehicle_data IS NULL)
    ...
    -- 45 em diante (Append-Only): Propósito e Campos de Pacote de Créditos
    ...
    CASE
      WHEN (pt.purpose = 'credit_package' OR pt.credit_package_order_id IS NOT NULL)
