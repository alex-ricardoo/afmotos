...
const statusConfig: Record<string, { label: string; classes: string; dotClass: string }> = {
  AVAILABLE: {
    label: 'Disponível',
    classes:
      'bg-[#c9a44c]/15 text-[#e3c56c] border-[#c9a44c]/40 shadow-[0_0_8px_rgba(201,164,76,0.2)]',
    dotClass: 'bg-[#e3c56c]',
  },
  disponivel: {
    label: 'Disponível',
  ...
  RESERVED: {
    label: 'Reservada',
    classes:
      'bg-[#b8bcc2]/15 text-[#e6e8eb] border-[#b8bcc2]/40 shadow-[0_0_8px_rgba(184,188,194,0.15)]',
    dotClass: 'bg-[#b8bcc2]',
  },
  reservada: {
    label: 'Reservada',
  ...
  RENTED: {
    label: 'Alugada',
    classes:
      'bg-[#8f6d25]/20 text-[#e3c56c] border-[#8f6d25]/50 shadow-[0_0_8px_rgba(143,109,37,0.2)]',
    dotClass: 'bg-[#c9a44c]',
  },
  alugada: {
    label: 'Alugada',
...
export function MotorcycleStatusBadge({ status, className }: StatusBadgeProps) {
  const normalizedKey = status ? String(status).trim() : 'disponivel';
  const config = statusConfig[normalizedKey] ||
    statusConfig[normalizedKey.toUpperCase()] ||
    statusConfig[normalizedKey.toLowerCase()] || {
      label: status,
